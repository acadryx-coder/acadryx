...
// FEATURE CARD (icon + label for dashboard grid)
export function FeatureCard({ icon, label, sub, onClick, badge }) {
  return (
    <Card onClick={onClick} style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: "12px", position: "relative", minHeight: 100 }}>
      {badge && (
        <div style={{
...
// AVATAR
export function Avatar({ initials, size = 36, src }) {
  return (
    <div style={{
      width: size, height: size, borderRadius: "50%",
      background: src ? "transparent" : "var(--brand)",
      display: "flex", alignItems: "center", justifyContent: "center",
...
// BTN
export function Btn({ children, onClick, variant = "primary", size = "md", style: extraStyle = {}, disabled }) {
  const base = {
...
export function Divider({ label }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "16px 0" }}>
      ...
      {label && <span style={{ fontSize: 12, color: "var(--text-muted)", fontWeight: 500 }}>{label}</span>}
      <div style={{ flex: 1, height: 1, background: "var(--border)" }} />
    </div>
  );
}
